...
class NotesRepository(
    ...
    /**
     * Invoked after any note write. Home screen widgets cannot observe a Flow — they are only
     * redrawn when told to — so the hook lives here, where every write path passes, rather than
     * being repeated at each of the ViewModel's dozen mutators.
     */
    ...
    suspend fun insertNote(note: Note) = noteDao.insertNote(note).also { onNotesChanged() }
    suspend fun updateNote(note: Note) = noteDao.updateNote(note).also { onNotesChanged() }
    suspend fun deleteNote(note: Note) = noteDao.deleteNote(note).also { onNotesChanged() }
