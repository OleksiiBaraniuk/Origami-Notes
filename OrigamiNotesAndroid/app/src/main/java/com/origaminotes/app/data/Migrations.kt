...
/**
 * v4 → v5: add lastOpenedAt to notes for the "Last Open" sort mode.
 * Seeded from modifiedAt — the closest thing to "when you last touched this note" that
 * pre-v5 installs recorded. A zero default would bunch every existing note together.
 */
val MIGRATION_4_5 = object : Migration(4, 5) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `notes` ADD COLUMN `lastOpenedAt` INTEGER NOT NULL DEFAULT 0")
        db.execSQL("UPDATE `notes` SET `lastOpenedAt` = `modifiedAt`")
    }
}

/**
 * v5 → v6: add the block structure column.
 *
 * Left NULL on purpose — existing rows keep their plain `content`, and NoteBlocks.decode()
 * wraps that into a single text block on first open. No content is rewritten here, so a bad
 * conversion can't damage existing notes.
 */
val MIGRATION_5_6 = object : Migration(5, 6) {
    override fun migrate(db: SupportSQLiteDatabase) {
        db.execSQL("ALTER TABLE `notes` ADD COLUMN `blocks` TEXT")
    }
}

/** v6 → v7: add the note type. Every existing note is a text note. */
