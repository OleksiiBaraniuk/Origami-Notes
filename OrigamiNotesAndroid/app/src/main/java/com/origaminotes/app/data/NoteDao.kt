...
@Dao
interface NoteDao {
    @Query("SELECT * FROM notes WHERE folderId IS NULL ORDER BY isPinned DESC, sortOrder DESC")
    fun getUncategorizedNotes(): Flow<List<Note>>

    @Query("SELECT * FROM notes WHERE folderId = :folderId ORDER BY isPinned DESC, sortOrder DESC")
